...
export default function TurmasPage() {
  ...
  const estadoInicial = {
    codigoTurma: '', id_curso: '', diasSemana: '', horarioInicio: '', horarioFim: '', limiteVagas: '', semestreAno: '', status: 'Aberta', idProfessor: ''
  };
  ...
  const turmasFiltradas = turmas.filter(turma =>
    ...
    turma.semestreAno?.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-3 gap-10 mb-16">
        ...
        <div className="bg-zinc-200 p-8 border-4 border-zinc-900 shadow-[8px_8px_0px_0px_rgba(24,24,27,1)] flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-4 mt-10">
            <button type="submit" className="bg-yellow-400 text-zinc-900 p-4 font-black uppercase tracking-[0.2em] hover:bg-zinc-900 hover:text-white transition-all border-2 border-zinc-900">
              Abrir Turma
            </button>
      ...
      <section className="bg-white border-4 border-zinc-900 shadow-[12px_12px_0px_0px_rgba(24,24,27,1)] overflow-hidden">
        <div className="bg-zinc-900 p-6 flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="relative w-full max-w-md">
            <input
              type="text" 
              placeholder="PESQUISAR CÓDIGO OU SEMESTRE..."
              value={searchTerm} 
        ...
        <div className="overflow-x-auto p-4">
          <table className="w-full text-left min-w-[1000px]">
            ...
            <tbody className="font-bold text-base">
              {loading && <tr><td colSpan="6" className="p-10 text-center uppercase italic font-black text-zinc-600 animate-pulse">Connecting...</td></tr>}
              ...
              {!loading && !error && turmasFiltradas.map(turma => (
                <tr key={turma.id_turma} className="border-b-2 border-zinc-100 hover:bg-yellow-50 transition-colors group">
                  ...
                  <td className="p-4 text-zinc-900 uppercase">
                    {turma.curso ? turma.curso.nome : 'Sem Curso Atrelado'}
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-center">
                    <span className={`px-3 py-1.5 font-black text-xs uppercase border ${
                      turma.status === 'Aberta' ? 'bg-emerald-100 text-emerald-700 border-emerald-300' :
                      turma.status === 'Em Andamento' ? 'bg-blue-100 text-blue-700 border-blue-300' :
