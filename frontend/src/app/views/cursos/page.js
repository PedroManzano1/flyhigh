...
export default function CursosPage() {
  ...
  // Ativo inicializado como 'true' (string) para lidar melhor com o Select no React
  const estadoInicial = {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      ativo: formData.ativo === 'true' // Convertendo string do select para boolean
    };
  ...
  const handleEditar = (id) => {
    router.push(`/views/cursos/editar?id=${id}`); // Ajuste a rota conforme sua estrutura de pastas
  };
        ...
        <div className="bg-yellow-400 p-8 border-4 border-zinc-900 shadow-[8px_8px_0px_0px_rgba(24,24,27,1)] flex flex-col justify-between">
          <div>
            ...
            <p className="font-bold text-zinc-900 text-sm leading-tight border-l-4 border-zinc-900 pl-4">
              Defina com clareza o nome e o tipo do nível. O valor base será utilizado como referência no momento da matrícula dos alunos.
            </p>
          </div>
      ...
      <section className="bg-white border-4 border-zinc-900 shadow-[12px_12px_0px_0px_rgba(24,24,27,1)] overflow-hidden">
        ...
        <div className="bg-zinc-900 p-6 flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="relative w-full max-w-md">
            <input
              type="text" 
              placeholder="PESQUISAR NOME DO NÍVEL OU TIPO..."
              value={searchTerm} 
          ...
          <table className="w-full text-left min-w-[1000px]">
            <thead>
              <tr className="border-b-4 border-zinc-900 bg-gray-50">
                <th className="p-4 font-black uppercase text-[10px] tracking-widest text-zinc-500">ID Nível</th>
                <th className="p-4 font-black uppercase text-[10px] tracking-widest text-zinc-900">Nome / Tipo</th>
                <th className="p-4 font-black uppercase text-[10px] tracking-widest text-zinc-900">Público</th>
                <th className="p-4 font-black uppercase text-[10px] tracking-widest text-zinc-900">Carga Horária</th>
                <th className="p-4 font-black uppercase text-[10px] tracking-widest text-zinc-900">Valor Base</th>
                <th className="p-4 font-black uppercase text-[10px] tracking-widest text-zinc-900 text-center">Status</th>
            ...
            <tbody className="font-bold text-sm">
                ...
                <tr key={curso.id_curso} className="border-b-2 border-zinc-100 hover:bg-yellow-50 transition-colors group">
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <div className="flex flex-col">
                      <span className="uppercase text-zinc-900">{curso.nome}</span>
                      <span className="text-[10px] text-zinc-400 font-mono italic">TIPO: {curso.tipo}</span>
                    </div>
                  </td>
                  ...
                  <td className="p-4 font-mono text-zinc-900">
                    {curso.valorBase 
                      ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(curso.valorBase) 
                      : '---'}
                  </td>
                  <td className="p-4 text-center">
                    {curso.ativo ? (
                      <span className="bg-emerald-100 text-emerald-700 px-2 py-1 font-black text-[10px] uppercase border border-emerald-300">Ativo</span>
                    ) : (
                      <span className="bg-red-100 text-red-700 px-2 py-1 font-black text-[10px] uppercase border border-red-300">Inativo</span>
                    )}
                  </td>
                  <td className="p-4">
                    <div className="flex justify-center gap-2">
                      <button 
                        onClick={() => handleEditar(curso.id_curso)} 
                      ...
                      <button 
                        onClick={() => handleExcluir(curso.id_curso)} 
              ...
              {!loading && !error && cursosFiltrados.length === 0 && (
                 <tr><td colSpan="7" className="p-8 text-center text-zinc-400 uppercase font-black italic">Nenhum nível encontrado.</td></tr>
              )}
            </tbody>
          </table>
        </div>
